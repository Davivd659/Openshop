$(function() {
	function a() {
		...
		if (!s1) return void msgalert("请填写标题");	
		if (!a) return void msgalert("请填写您的称呼");
		...
		if ($(".zxd-form-tip").hide(), 0 < q.length && "" == $.trim(q.val())) return msgalert("请填写您的小区名"), !1;
		var t = $.trim($(p).val());
		if (0 < p.length) {
			if (!/^20\d{2}[0|1]\d{1}[0-3]\d$/.test(t)) return msgalert("请填写正确的时间"), !1;
			var u = "",
				v = new Date,
				w = v.getFullYear(),
				x = v.getMonth() + 1,
			...
			if (D = 2 == z ? 0 == w % 4 && 0 != w % 100 && w % 200 != 0 && w % 300 != 0 && w % 400 != 0 ? 29 : 28 : 4 == z || 6 == z || 9 == z || 11 == z ? 30 : 31, u += v.getFullYear().toString(), u += 1 == x.toString().length ? "0" + x.toString() : x.toString(), u += 1 == y.toString().length ? "0" + y.toString() : y.toString(), parseInt(u) > parseInt(t)) return msgalert("验房时间不能小于当天"), !1;
			if (t.substr(4, 1) >= 1 && 2 < t.substr(5, 1)) return msgalert("请填写正确的时间"), !1;
			if (D < parseInt(t.substr(6, 2))) return msgalert("请填写正确的时间"), !1
		}
		$.ajax({
			type: "POST",
			url: i,
			...
			success: function(a) {
				...
				if (1 == a.status || 5 == a.status) {
					if (1 == a.status) {
						c = "恭喜您，发布成功！", d = "开店ing客服将在24小时内与您电话联系，审核信息真实性，以便安排服务。", h = a.tmpYid;
					...
					} else c = "很抱歉", d = "您所在的城市尚未开通此项服务，我们会在开通后第一时间为您服务。";
					e.html(c), f.html(d), $(".zxd-mask, .zxd-dialog").show(), b()
	...
	function f() {
		var a = 400 / 864e5,
			b = (new Date).getTime(),
			c = Math.round(a * b);
		$("#zxb_addamount").html(c - 61e5), setTimeout("autoAddZxbNumber()", 18e4)
	}
	...
	function g(a) {
		var b, c = null,
			...
			e = a + "=",
			f = d.indexOf(e);
		return -1 != f && (f += e.length, b = d.indexOf(";", f), c = d.substring(f, b)), c
	}
	var h = 0;
	$(".mfsj-cost").change(function() {
		var a = $(this).val();
		a ? $(this).next().hide() : $(this).next().show()
	}), $(".zxd-mask").height($(document).height()), $("#zxd-tel").blur(function() {
		var a = /^(1[3|4|5|7|8])[\d]{9}$/,
			b = $("#zxd-tel").val();
		a.test(b) ? $(".zxd-form-tip").hide() : $(".zxd-form-tip").show(), $("#zxd-tel").val() || $(".zxd-form-tip").hide()
	}), $("#zxd-form-submit").click(function(b) {
		b.preventDefault(), a()
	}), $("#zxd-dialog-ok").on("touchend", function(a) {
	...
	$(window).scroll(function() {
		e()
	}), e(), $('input[name="ptag"]').buriedPoint(), f()
