...
namespace YG.SC.WebUI.Controllers
{
    public class ProductController : WebBaseController
    {
        ...
        {
            string Viewname = "";
            var fileCICName = UploadImgUtility.UploadImage(Request.Files["Imagefile"], Server.MapPath(CommonContorllers.FileUploadProductImgPath));
            if (!string.IsNullOrEmpty(fileCICName))
            {
                moudel.Image = CommonContorllers.FileUploadProductImgPath + fileCICName;
