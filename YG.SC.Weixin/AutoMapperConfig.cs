...
using YG.SC.WeiXin.Controllers;
    ...
    public abstract class AutoMapperConfig
    {
        public static void RegisterMappings()
        {
            ...
            Mapper.CreateMap<ShopProject, ProjectDetailsViewModel>()
                ...
                .ForMember(src => src.Region, det => det.MapFrom(src => GetShopAttributeValues(src.ShopProjectMain.FirstOrDefault(m => m.ShopProjectId == src.Id).RegionId.Value.ToString())))
                .ForMember(src => src.RentalPrice, det => det.MapFrom(src => src.ShopProjectMain.FirstOrDefault(p => p.ShopProjectId == src.Id).RentalPrice.Value))
                .ForMember(src => src.SalePrice, det => det.MapFrom(src => src.ShopProjectMain.FirstOrDefault(p => p.ShopProjectId == src.Id).SalePrice.Value))
                ...
                // 出租、出售
                .ForMember(src => src.State, det => det.MapFrom(src => GetShopAttributeValues(src.ShopProjectMain.FirstOrDefault(m => m.ShopProjectId == src.Id).StateId)))
                .ForMember(src => src.StateId,m=> m.MapFrom(src =>src.ShopProjectMain.FirstOrDefault( p=> p.ShopProjectId == src.Id).StateId))
                // 行业
                .ForMember(src => src.Trade, det => det.MapFrom(src => GetShopAttributeValues(src.ShopProjectMain.FirstOrDefault(m => m.ShopProjectId == src.Id).TradeId)))
                ...
                // 商圈
                .ForMember(src => src.TradingArea, det => det.MapFrom(src => GetShopAttributeValues(src.ShopProjectMain.FirstOrDefault(m => m.ShopProjectId == src.Id).TradingAreaId.Value.ToString())))
                //业态
