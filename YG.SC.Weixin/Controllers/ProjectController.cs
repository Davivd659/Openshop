...
namespace YG.SC.WeiXin.Controllers
{
    public class ProjectController : BaseController
    {
        ...
        public ProjectController(IProjectPhotoService iProjectPhotoService,
            ...
            ILinkService ilinkService, IShopAdPositionService iShopAdPositionService
            )
        {
        ...
        public ActionResult List()
        {
            ...
            {
                var ProjectDbList = _iProjectTeamService.GetAll().Where(p => p.ShopProjectId == item.Id && p.Status == 1).ToList();
                var projectServiceList = Mapper.Map<List<ProjectService>, List<ProjectServiceViewModel>>(ProjectDbList);
                item.ProjectServiceViewModels = projectServiceList;
            }
        ...
        public ActionResult Index(int id)
        {
            ...
            // 项目图片
            int pg = 1;
            var projectPhoto = this._iProjectPhotoService.GetEntitsByImageName(pg, projectId, string.Empty);
            ViewBag.PhotosList = projectPhoto.Item1;
            // 登录信息 
        ...
        [HttpPost]
        public ActionResult ApplyCustomer(ProjectApplyModel model)
        {
            var applyActivity = new ApplyActivity
            {
                ...
                Type = "个人",
               GrouppurchaseId = model.ProjectId
            };
            applyActivity.Status = 1;
            applyActivity.UpdateDate = DateTime.Now;
        ...
        [HttpGet]
        [ActionName("ApplyCustomer")]
        public ActionResult ApplyCustomer(int projectId)
        {   
            ViewBag.ProjectId = projectId;
            ViewBag.ApplyCount = _iApplyActiviteService.GetApplyCount(projectId);
            return View();
            return View();
        }
        public ActionResult Search()
        {
            ...
            int IDPriceSale = GetID(str_PriceSale);
            int IDStatus = GetID(str_Status);
            int IDOpenTime = GetID(str_OpenTime);
            int IDPrice = 0;
            if (IDStatus == 145)
            {
                //  出租 
            ...
            string queryString = Request.Url.PathAndQuery;
            //ViewBag.Filter = FilterBuilder(IDIndustry, IDQuYu, IDSubWay, IDType, IDPrice, IDStatus, IDOpenTime, queryString);
            // search 
            int PageIndex = 1;
            int.TryParse(Request.QueryString["PageIndex"], out PageIndex);
            var searchCriteria = new ProjectSearchCriteria();
            ...
            searchCriteria.PriceSaleId = IDPriceSale;
            searchCriteria.StatusID = IDStatus;
            searchCriteria.OpenTimeId = IDOpenTime;
            searchCriteria.Keys = Request["keys"];
            ...
            , int? status, int opentime, int? PageIndex)
        {
            ...
            searchCriteria.OpenTimeId = opentime;
            searchCriteria.Keys = keys;
            var searchResult = _iShopProjectService.SearchProject(searchCriteria);
